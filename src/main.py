...
def start_message(message):
    bot.send_message(
        message.from_user.id, "Привет, это реализация алгоритма Диффи-Хеллмана"
    )
...
def get_name2(message):
    ...
    bot.send_message(
        message.from_user.id, "Введите целое число N для функции\nF(x)= N^x mod M"
    )
...
def get_N(message):
    ...
    try:
        int(N)
        bot.send_message(
            message.from_user.id, "Введи целое число M для функции\nF(x)= N^x mod M"
        )
...
def loop_for_N(message):
    bot.send_message(
        message.from_user.id, "Необходимо ввести целое число(!)\nПовторите ввод"
    )
    bot.register_next_step_handler(message, get_N)
...
def get_M(message):
    ...
    try:
        ...
        bot.send_message(
            ...
            + " и "
            + abonent2
            + " договорились о том, что число N будет равно "
            + N
            + ", а число M равно "
            + M,
        )
        bot.send_message(
            message.from_user.id,
            "Введите случайное целое число p для абонента "
            + abonent1
            + "\nФормула: P= N^p mod M",
        )
...
def get_p(message):
    ...
    try:
        ...
        bot.send_message(
            message.from_user.id,
            "Число P по формуле будет равно"
            + "\nP= "
            + N
            + "^"
            + p
        ...
        bot.send_message(
            message.from_user.id,
            abonent1 + " отправляет абоненту " + abonent2 + " число P ",
        )
        bot.send_message(
            message.from_user.id,
            " Введите случайное целое число r для абонента "
            + abonent2
            + "\nФормула: R= N^r mod M",
        )
...
def get_r(message):
    ...
    try:
        ...
        bot.send_message(
            message.from_user.id,
            "Число R по формуле будет равно"
            + "\nR= "
            + N
            + "^"
            + r
        ...
        bot.send_message(
            message.from_user.id,
            abonent2 + " отправляет абоненту " + abonent1 + " число R",
        )
        ...
        bot.send_message(
            message.from_user.id, "Вы хотите снова запустить алгоритм? (Да/Нет)"
        )
    except:
        loop_for_r(message)
